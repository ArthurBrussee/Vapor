...
public class Vapor2Editor : Editor {
	...
	private static void RenderCustomLightGizmo(Vapor vapor, GizmoType gizmoType) {
		...
		Vector3 sc1 = vapor.NoiseBaseLayer.SetScale;
		Vector3 sc2 = vapor.NoiseSecondaryLayer.SetScale;
		Vector3 sc3 = vapor.NoiseDetailLayer.SetScale;

		Vector3 scroll1 = vapor.NoiseBaseLayer.ScrollSpeed;
		Vector3 scroll2 = vapor.NoiseSecondaryLayer.ScrollSpeed;
		Vector3 scroll3 = vapor.NoiseDetailLayer.ScrollSpeed;


		float str1 = vapor.NoiseBaseLayer.Strength;
		float str2 = vapor.NoiseSecondaryLayer.Strength;
		float str3 = vapor.NoiseDetailLayer.Strength;
		...
		if (s_visualizeMode == VisualizeMode.Layers) {
			...
			DrawNoiseVisualize(vapor.NoiseBaseLayer, position - scroll1 * time);
			DrawNoiseVisualize(vapor.NoiseSecondaryLayer, position - scroll2 * time);
			DrawNoiseVisualize(vapor.NoiseDetailLayer, position - scroll3 * time);
		}else if (s_visualizeMode == VisualizeMode.Total) {

			s_noiseVisualizeMaterial.SetTexture("_NoiseTex0", vapor.NoiseBaseLayer.NoiseTexture);
			s_noiseVisualizeMaterial.SetTexture("_NoiseTex1", vapor.NoiseSecondaryLayer.NoiseTexture);
			s_noiseVisualizeMaterial.SetTexture("_NoiseTex2", vapor.NoiseDetailLayer.NoiseTexture);



			s_noiseVisualizeMaterial.SetVector("_NoiseScale0", vapor.NoiseBaseLayer.SetInvScale);
			s_noiseVisualizeMaterial.SetVector("_NoiseScale1", vapor.NoiseSecondaryLayer.SetInvScale);
			s_noiseVisualizeMaterial.SetVector("_NoiseScale2", vapor.NoiseDetailLayer.SetInvScale);


			float time = (float)EditorApplication.timeSinceStartup;

			s_noiseVisualizeMaterial.SetVector("_NoiseScroll0", vapor.NoiseBaseLayer.SetScaledScrollSpeed * time);
			s_noiseVisualizeMaterial.SetVector("_NoiseScroll1", vapor.NoiseSecondaryLayer.SetScaledScrollSpeed * time);
			s_noiseVisualizeMaterial.SetVector("_NoiseScroll2", vapor.NoiseDetailLayer.SetScaledScrollSpeed * time);
